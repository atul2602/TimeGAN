...
import numpy as np
from utils2 import extract_time, random_generator, batch_generator
...
def timegan(ori_data, parameters):
    ...
    for itt in range(iterations):
        ...
        G_loss_S = 0.1 * torch.sqrt(nn.MSELoss()(H[:, 1:, :], H_hat_supervise[:, :-1, :]))
        E_loss = E_loss0 + G_loss_S
    ...
    for itt in range(iterations):
        ...
        for kk in range(2):
            ...
            G_loss_U = criterion(Y_fake, torch.ones((batch_size, max_seq_len, 1)))
            G_loss_U_e = criterion(Y_fake_e, torch.ones((batch_size, max_seq_len, 1)))
            G_loss_S = torch.sqrt(mse_loss(H[:, 1:, :], H_hat_supervise[:, :-1, :]))
        ...
        D_loss_real = criterion(Y_real, torch.ones((batch_size,max_seq_len, 1)))
        D_loss_fake = criterion(Y_fake, torch.zeros((batch_size,max_seq_len, 1)))
        D_loss_fake_e = criterion(Y_fake_e, torch.zeros((batch_size,max_seq_len, 1)))
...
# convert compiled tf to dynamic